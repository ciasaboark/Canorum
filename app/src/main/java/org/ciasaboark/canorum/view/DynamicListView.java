...
import org.ciasaboark.canorum.adapter.StableArrayAdapter;
...
public class DynamicListView<T> extends ListView {
    ...
    private ListViewReorderListener<T> mListener;
    ...
    public void setListViewReorderListener(ListViewReorderListener<T> listener) {
        mListener = listener;
    }
    ...
    private void handleCellSwitch() {
        ...
        if (isBelow || isAbove) {
            ...
            swapElements(mData, originalItem, getPositionForView(switchView));

            ((QueueAdapter) getAdapter()).notifyDataSetChanged();
    ...
    private void swapElements(List<T> arrayList, int indexOne, int indexTwo) {
        ...
        if (mListener != null) {
            mListener.onListReordered(arrayList);
        }
    }
    ...
    public interface ListViewReorderListener<T> {
        public void onListReordered(List<T> list);
    }
}